...
* must expose a JSON API that allows a user to both read and write data
* must filter data appropriately in both the web and API based on authorization rules
* must create a Ruby gem which allows a user to easily interact with your API
* must make use of a Virtual Private Server to host your application
* must automate the deployment of your application to a single command
* application must accept uploads of some user images
* application must manipulate those images to suit the layout/style of the app itself
